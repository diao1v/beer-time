...
// FIRMWARE_VERSION is supplied by platformio.ini / CI build flag.
// Fallback used when neither is set (e.g. someone strips the build_flags).
#ifndef FIRMWARE_VERSION
#define FIRMWARE_VERSION "dev-local"
#endif
// drawIdleFrame uses clock_bg's macros + arrays directly, so we need its
// header at the top. Celebration animations + wrappers + registry macro
// live in include/animations/_registry.h, which is generated by
// tools/build_animations.py and #included further below, AFTER drawGifFrame.
...
// drawX wrappers and ASSET_ANIMATION_ENTRIES live here; this include must
// come after drawGifFrame because the wrappers reference it.
...
static const Animation ANIMATIONS[] = {
  { "fireworks", drawFireworks, false },   // ANIMATIONS[0] = default fallback
...
static const Animation *lookupAnimation(const String &name) {
  ...
  return &ANIMATIONS[0];  // unknown name → first entry (fireworks)
}
